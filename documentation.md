...
The application connects to MongoDB using Mongoose. The connection logic is encapsulated in `lib/db.ts`. It implements a caching mechanism to prevent multiple connections during hot reloads in development.

### Models

Mongoose schemas and models are defined in the `models/` directory. Each file typically defines an interface for the document and its corresponding Mongoose schema and model. The `models/index.ts` file re-exports all models for easier import.
...
API routes are implemented using Next.js Route Handlers (`app/api/`). They are RESTful endpoints that handle data interactions.
...
Frontend pages are built with React components within the Next.js App Router (`app/`).
